...
def get_population():
    data = pd.read_csv('../vaccine_allocation/AgeDistrictPopulation.csv', sep = ' ', header=None)
...
def generate_plots_comparison(outputfile1_case1,outputfile2_case1,outputfile3_case1,  start_index, plot_days_data, plot_days_simulation, smooth_days, sub_dir, start_offset):
    ...
    district_simulation_1_case1 = prepare_district_simulation_plots_susceptible(outputfile1_case1,plot_days_simulation)
    district_simulation_2_case1 = prepare_district_simulation_plots_susceptible(outputfile2_case1,plot_days_simulation)
    district_simulation_3_case1 = prepare_district_simulation_plots_susceptible(outputfile3_case1,plot_days_simulation)
    ...
    for x in districts:
        population = population_df.loc[population_df['District']==x, 'pop'].item()
    ...
    plt.plot((karnataka_curve1_case1/population), 'b',label='Baseline')
    plt.plot((karnataka_curve2_case1/population), 'g',label=label_names[parameter_case] + ' + 10%')
    plt.plot((karnataka_curve3_case1/population), 'c',label=label_names[parameter_case] + ' - 10%')
 
    plt.xticks(x_indices, x_labels, rotation='vertical')
    plt.grid(True)
    plt.ylabel('Susceptible Fraction') 
    plt.title('Karnataka -- Susceptibles ('+mobility_names[mobility_case]+')') 
    plt.legend()
    plt.ylim(0,1)
    plt.savefig(sub_dir+'/daily_Karnataka'+label_names[parameter_case]+'_'+mobility_case+'.png', bbox_inches='tight')    
    # plt.show()
    plt.close()
...
for i in range(0,4):
    for j in range(0,3):
        ...
        if closed_loop==1:
            ...
            path1 = '../../vaccine_allocation_until28Feb/fullrun_'+mobility_case+'/167000/'
        ...
        generate_plots_comparison(output1_case1,output2_case1,output3_case1 , 204, 509, 509, 7, 'susceptible_fraction', 20)
