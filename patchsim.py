...
def load_Theta(configs, patch_df):
    ...
    if configs["NetworkType"] == "Static":
        ...
        if not list(sorted(set(theta_df.theta_index))) == list(range(73)):
...
def do_patchsim_det_force_step_betaoutside(State_Array, patch_df, params, theta, seeds, vaxs, t):
    """Do step of the deterministic simulation."""
    S, E, I, R, V, new_inf = State_Array  ## Aliases for the State Array
    ...
    # Infection force
    inward_contacts = theta.T.dot(beta_j_eff)

    inf_force = inward_contacts * params["beta"][:, t]
...
def do_patchsim_det_force_step_betaoutside_waning(State_Array, patch_df, params, theta, seeds, vaxs, t, wane_array_recoveries, wane_array_vaccinations):
    ...
    # New exposures during day t
    new_inf[t] = params["alpha"] * E[t]
    
    new_exposed = np.minimum(inf_force * S[t], S[t])
    
    # Update to include presymptomatic and asymptomatic terms
    S[t + 1] = S[t] - new_exposed + params["delta"] * R[t]
    E[t + 1] = new_exposed + (1 - params["alpha"]) * E[t]
    I[t + 1] = params["alpha"] * E[t] + (1 - params["gamma"]) * I[t]
    R[t + 1] = params["gamma"] * I[t] + (1 - params["delta"]) * R[t]
    V[t + 1] = V[t]
    ...
    move_people_recoveries = np.zeros(len(patch_df)).astype(int)
    move_people_vaccinations = np.zeros(len(patch_df)).astype(int)
    #print('Wane array:', wane_array_recoveries[0])
    for i in range(0,len(new_recoveries)):
        move_people_recoveries[i] = np.count_nonzero(wane_array_recoveries[i]==0)
    ...
    S[t+1] = S[t+1] + (move_people_recoveries)
    R[t+1] = R[t+1] - (move_people_recoveries)
...
def run_disease_simulation(
    ...
    #print(State_Array[:,0,:]) # variables , time_index, patch_index
    if configs["NetworkType"] == "Static":
        ...
        wane_array_recoveries=[(120*np.random.weibull(3.67,int(State_Array[3,0,i]))).astype(int) for i in range(0,len(patch_df))]
        wane_array_vaccinations=[(120*np.random.weibull(3.67,int(State_Array[5,0,i]))).astype(int) for i in range(0,len(patch_df))]
        for t in range(params["T"]):
            #print('day:', t)
    ...
    population_states = ['susceptible', 'exposed', 'infected', 'recovered', 'vaccinated', 'newexposure']
    cur_dir = os.getcwd()
    index = patch_df.id
    columns = np.arange(int(configs["Duration"]))

    for i in range(6):
        save_matrix = np.round(State_Array[i][:-1, :].T).astype(int)
        out_df = pd.DataFrame(index=index, columns=columns, data=save_matrix)
        out_df.to_csv(F"{cur_dir}/{configs['OutputFile']}_{population_states[i]}.csv", header=None, sep=" ")
